...
class _CouponScreenState extends State<CouponScreen> {
  ...
  Future<List<UserCoupon>> _fetchCoupons() async {
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await _fetchCoupons();
      if (!mounted) return;
      setState(() {
        _coupons = list;
        _loading = false;
      ...
      setState(() {
        _loading = false;
        _error = loadErrorMessage(e, fallback: '쿠폰을 불러오지 못했습니다.');
      });
    }
  }

  void _onMockRedeemed(String id) {
    setState(() => _pendingMockIds.add(id));
    _load();
  }
  ...
  Widget _buildBody() {
    ...
    if (_error != null && _coupons.isEmpty) {
      return CustomScrollView(
        ...
        slivers: [
          const SliverToBoxAdapter(child: _CouponIntroSection()),
                ...
                (context, index) {
                  ...
                  return Padding(
                    padding: EdgeInsets.only(
                      bottom: index < _coupons.length - 1 ? 12 : 0,
                    ),
                    child: _CouponTicketCard(
                      coupon: c,
                      onRedeemed: () {
                        if (c.id.startsWith('mock_')) {
                          _onMockRedeemed(c.id);
                        } else {
                          _load();
                        }
                      },
...
class _CouponTicketCardState extends State<_CouponTicketCard> {
  ...
  Future<void> _redeem() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(loadErrorMessage(e, fallback: '쿠폰 신청에 실패했습니다.')),
  ...
  Color get _brandColor {
    switch (widget.coupon.brand) {
      case CouponBrand.mega:
        return const Color(0xFFFFC107);
      case CouponBrand.chicken:
        return const Color(0xFFC62828);
      case CouponBrand.convenience:
        return const Color(0xFF1565C0);
      case CouponBrand.giftcard:
        return const Color(0xFF6A1B9A);
      case CouponBrand.other:
  ...
  Widget build(BuildContext context) {
    ...
    final thumbUrl = resolveMediaUrl(coupon.imageUrl);
                  ...
                  child: Row(
                    children: [
                            ...
                            Container(
                              ...
                              child: SizedBox(
                                ...
                                child: thumbUrl != null
                                    ...
                                    : PhosphorIcon(_brandIcon, color: Colors.white, size: 22),
                              ),
                            ),
                            const Gap(8),
                            Text(
                      ...
                      SizedBox(
                        width: 18,
                        child: CustomPaint(
                          painter: _DashLinePainter(color: Colors.grey.shade200),
                        ),
                      ),
                      Expanded(
                        child: Padding(
